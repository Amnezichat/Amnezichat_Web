...
// Conversion to bytes for Signature
impl Signature {
    ...
    /// Convert from bytes to Signature
    pub fn from_bytes(sig: &Sig, bytes: &[u8]) -> Option<Signature> {
        if bytes.len() <= sig.length_signature() {
            Some(Signature {
                bytes: bytes.to_vec(),
            })
        } else {
...
impl PublicKey {
    ...
    /// Constructs a `PublicKey` object from raw bytes.
    ///
    /// This function checks that the provided raw bytes have the correct length
    /// for the public key and returns a `PublicKey` object if valid.
    pub fn from_bytes(sig: &Sig, bytes: &[u8]) -> Option<Self> {
        // Ensure that the length of the provided bytes matches the expected length for the public key.
        let expected_length = sig.length_public_key();
        if bytes.len() == expected_length {
            Some(PublicKey { bytes: bytes.to_vec() })
        } else {
            None
        }
    }

    /// Constructs a `PublicKey` from raw bytes and returns a Result.
    pub fn from_bytes_result(sig: &Sig, bytes: &[u8]) -> Result<Self> {
        match PublicKey::from_bytes(sig, bytes) {
...
impl SecretKey {
    ...
    /// Constructs a `SecretKey` object from raw bytes.
    ///
    /// This function checks that the provided raw bytes have the correct length
    /// for the secret key and returns a `SecretKey` object if valid.
    pub fn from_bytes(sig: &Sig, bytes: &[u8]) -> Option<Self> {
        // Ensure that the length of the provided bytes matches the expected length for the secret key.
        let expected_length = sig.length_secret_key();
        if bytes.len() == expected_length {
            Some(SecretKey { bytes: bytes.to_vec() })
        } else {
            None
        }
    }

    /// Constructs a `SecretKey` from raw bytes and returns a Result.
    pub fn from_bytes_result(sig: &Sig, bytes: &[u8]) -> Result<Self> {
        match SecretKey::from_bytes(sig, bytes) {
