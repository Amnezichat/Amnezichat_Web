...
impl<'kem> PublicKeyRef<'kem> {
    /// Returns the raw bytes of the public key
    pub fn raw_bytes_kem(&self) -> &[u8] {
        self.bytes
    }

    /// Constructs a `PublicKeyRef` object from raw bytes.
    ///
    /// This function checks that the provided raw bytes have the correct length
    /// for the public key and returns a `PublicKeyRef` object if valid.
    pub fn from_bytes_kem(kem: &Kem, bytes: &'kem [u8]) -> Option<Self> {
        // Ensure that the length of the provided bytes matches the expected length for the public key.
        let expected_length = kem.length_public_key();
        if bytes.len() == expected_length {
            Some(PublicKeyRef::new(bytes))
        } else {
            None
        }
    }

    /// Constructs a `PublicKeyRef` from raw bytes and returns a Result.
    pub fn from_bytes_kem_result(kem: &Kem, bytes: &'kem [u8]) -> Result<Self> {
        match PublicKeyRef::from_bytes_kem(kem, bytes) {
            Some(pub_key_ref) => Ok(pub_key_ref),
            None => Err(Error::InvalidLength),
        }
    }
}

impl<'kem> SecretKeyRef<'kem> {
    /// Returns the raw bytes of the secret key
    pub fn raw_bytes_kem(&self) -> &[u8] {
        self.bytes
    }

    /// Constructs a `SecretKeyRef` object from raw bytes.
    ///
    /// This function checks that the provided raw bytes have the correct length
    /// for the secret key and returns a `SecretKeyRef` object if valid.
    pub fn from_bytes_kem(kem: &Kem, bytes: &'kem [u8]) -> Option<Self> {
        // Ensure that the length of the provided bytes matches the expected length for the secret key.
        let expected_length = kem.length_secret_key();
        if bytes.len() == expected_length {
            Some(SecretKeyRef::new(bytes))
        } else {
            None
        }
    }

    /// Constructs a `SecretKeyRef` from raw bytes and returns a Result.
    pub fn from_bytes_kem_result(kem: &Kem, bytes: &'kem [u8]) -> Result<Self> {
        match SecretKeyRef::from_bytes_kem(kem, bytes) {
            Some(secret_key_ref) => Ok(secret_key_ref),
            None => Err(Error::InvalidLength),
        }
    }
}
